...
class StreamingAudioRecovery:
    def __init__(self, pcap_file, output_dir="extracted_audio", use_whitelist=False, whitelist_ips=None, debug_mode=False, zmq_enabled=False, zmq_endpoint="tcp://127.0.0.1:5555", chunk_seconds=0.5):
        ...
        if self.zmq_enabled:
            try:
                ...
                # 设置高水位标记(HWM)，限制队列大小
                # 估算每条消息的平均大小，假设平均每条消息20KB
                # 1GB / 20KB = 51200条消息
                max_messages = 1024 * 1024 * 1024 // (self.chunk_bytes + 1024)  # 1GB除以(chunk大小+元数据估算)
                self.zmq_sock.set_hwm(max_messages)  # 设置队列最多存储消息数，约等于1GB
                logger.info(f"ZMQ队列设置: 最大消息数={max_messages}，估计总大小≈1GB")
                # 设置CONFLATE选项为True，使新消息覆盖旧消息
                self.zmq_sock.setsockopt(zmq.CONFLATE, 1)
                self.zmq_sock.connect(self.zmq_endpoint)
                logger.info(f"已连接ZMQ队列: {self.zmq_endpoint}, chunk时长: {self.chunk_seconds}s, chunk字节: {self.chunk_bytes}, HWM: {max_messages}, CONFLATE: 启用")
    ...
    def _search_pcap_header(self, input_stream):
        """在数据流中搜索有效的PCAP头，跳过损坏数据"""
        logger.info("智能搜索PCAP头，跳过损坏数据...")
        
        # PCAP文件头魔术数字（4字节）
        PCAP_MAGIC_NUMBERS = [
            b'\xa1\xb2\xc3\xd4',  # 大端序
            b'\xd4\xc3\xb2\xa1',  # 小端序  
            b'\xa1\xb2\x3c\x4d',  # 纳秒精度
            b'\x4d\x3c\xb2\xa1'   # 纳秒精度小端
        ]
        ...
        try:
            ...
            while True:
                ...
                # 检查缓冲区中是否有PCAP魔术数字
                for magic in PCAP_MAGIC_NUMBERS:
                    if magic in buffer:
                        # 找到可能的PCAP头
                        pos = buffer.find(magic)
                        if pos > 0:
                            logger.info(f"找到PCAP头，跳过 {pos} 字节损坏数据")
                        else:
                            logger.info("数据流开头就是有效的PCAP头")
                        return True
                
                # 如果缓冲区太大，保留最后4KB（足够包含魔术数字）
                if len(buffer) > 4096:
                    buffer = buffer[-4096:]
                    
            logger.info(f"搜索完成，共检查 {bytes_read} 字节数据，未找到PCAP头")
            return False
    ...
    def _publish_zmq(self, peer_ip, source, pcm_bytes, start_ts, end_ts, is_finished):
        ...
        try:
            ...
            total_msg_size = len(meta_bytes) + len(pcm_bytes)
            
            # 使用非阻塞发送，队列满时会丢弃旧数据
            import zmq as zmq_module  # 局部导入以解决未定义问题
            ...
            if self._msg_counter % 1000 == 0:
                logger.info(f"ZMQ队列状态: 已发送{self._msg_counter}条消息，当前消息大小={total_msg_size}字节")
        except zmq.Again:
            # 队列已满，消息被丢弃
            logger.warning(f"ZMQ队列已满，消息被丢弃: peer_ip={peer_ip}, source={source}, 大小={len(pcm_bytes)}字节")
        except Exception as e:
    ...
    def process_pcap_streaming(self):
        ...
        try:
            ...
            try:
                ...
                logger.info("✓ 初始pcap读取器创建成功")
            except Exception as e:
                logger.error(f"初始创建pcap读取器失败: {e}")
                logger.info("程序启动时FIFO中没有有效PCAP数据，将在循环中等待并搜索...")
            
            # 持续读取数据包
            while True:
                try:
                    logger.debug("=== 开始新的循环迭代 ===")
                    logger.debug(f"pcap_reader状态: {pcap_reader is not None}")
                    
                    # 检查pcap_reader是否需要重新创建
                    if pcap_reader is None:
                        logger.info("检测到pcap_reader为空，尝试重新创建...")
                        try:
                            pcap_reader = dpkt.pcap.Reader(input_stream)
                            logger.info("✓ pcap_reader重新创建成功")
                        except Exception as create_error:
                            logger.error(f"✗ pcap_reader重新创建失败: {create_error}")
                            
                            # 检测到损坏的PCAP头，尝试搜索并跳过损坏数据
                            if "invalid tcpdump header" in str(create_error):
                                logger.info("检测到损坏的PCAP数据，尝试搜索有效头...")
                                found = self._search_pcap_header(input_stream)
                                if found:
                                    logger.info("成功找到PCAP头，继续尝试创建reader")
                                    continue
                                else:
                                    logger.info("未找到PCAP头，等待新数据...")
                            
                            time.sleep(1.0)  # 稍微长一点的等待时间
                            continue
                    
                    # 非阻塞读取，使用select模拟
                    if self.pcap_file == '/dev/stdin':
                        logger.debug("准备执行select检查数据可用性...")
                        select_result = select.select([input_stream], [], [], 1.0)[0]
                        logger.debug(f"select结果: {bool(select_result)}")
                        if not select_result:
                            logger.debug("select超时，没有数据可读，进入超时检查...")
                            # 检查超时
                            ...
                            logger.debug("超时检查完成，continue到下次循环")
                            continue
                        else:
                            logger.debug("select检测到有数据可读，继续处理...")
                    
                    # 读取下一个数据包
                    logger.debug("准备调用 next(pcap_reader) 读取数据包...")
                    try:
                        logger.debug("正在执行 next(pcap_reader)...")
                        ts, buf = next(pcap_reader)
                        logger.debug(f"✓ 成功读取数据包: {len(buf)} 字节")
                    except StopIteration:
                        # 数据流可能暂时结束，等待新数据
                        logger.info("当前数据流已读取完毕，等待新数据...")
                        time.sleep(1)  # 短暂等待后继续尝试读取
                        continue  # 继续循环，不要退出
                    except Exception as e:
                        logger.error(f"读取数据包出错: {e}")
                        logger.info(f"异常类型: {type(e).__name__}, 异常消息: '{str(e)}'")
                        
                        # 如果是数据读取错误，可能是数据流损坏，尝试恢复
                        if "got 0" in str(e) or "needed at least" in str(e) or "invalid" in str(e):
                            logger.warning("✓ 匹配到数据流损坏模式，开始恢复流程...")
                            logger.info("尝试重新创建pcap读取器...")
                            try:
                                # 尝试重新创建pcap读取器
                                pcap_reader = dpkt.pcap.Reader(input_stream)
                                logger.info("✓ 数据流重新同步成功，继续处理")
                                continue
                            except Exception as sync_error:
                                logger.warning(f"✗ 重新同步失败: {sync_error}，等待新数据...")
                                time.sleep(1)
                                continue
                        else:
                            logger.info("✗ 不匹配数据流损坏模式，执行普通错误处理")
                        
                        logger.info("执行普通continue，跳到下次循环")
                        continue
                    ...
                    try:
                        ...
                        logger.info(f"[数据包解析阶段] 异常类型: {type(e).__name__}, 异常消息: '{str(e)}'")
                        
                        # 如果是数据解析错误，记录更多信息用于调试
                        if "got 0" in str(e) or "needed at least" in str(e):
                            logger.warning("✓ [数据包解析阶段] 匹配到PCAP解析错误模式")
                            logger.info("pcap_reader已损坏，标记为无效并等待数据流恢复...")
                            
                            # 将pcap_reader设为None，强制下次循环重新创建
                            pcap_reader = None
                            logger.info("✓ 已清理损坏的pcap_reader，下次循环将重新创建")
                            
                            # 等待一段时间让数据流稳定
                            time.sleep(1)
                            logger.info("[数据包解析阶段] 执行continue，跳到下次循环")
                        else:
                            logger.info("✗ [数据包解析阶段] 不匹配PCAP解析错误模式，执行普通错误处理")
                            logger.info("[数据包解析阶段] 执行普通continue，跳到下次循环")
                        continue
