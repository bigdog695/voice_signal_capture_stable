...
enable_audit_sigtrace() {
    ...
    $SUDO_BIN auditctl -a always,exit -F arch=b64 -S kill -S tkill -S tgkill -S rt_sigqueueinfo -k sigtrace_pcap 2>>"$MAIN_LOG"
    $SUDO_BIN auditctl -a always,exit -F arch=b32 -S kill -S tkill -S tgkill -S rt_sigqueueinfo -k sigtrace_pcap 2>>"$MAIN_LOG"
    AUDIT_ENABLED=1
    echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 已启用auditd规则(key=sigtrace_pcap)" | tee -a "$MAIN_LOG"
}
...
dump_audit_sigtrace() {
    ...
    # 导出原始记录
    $SUDO_BIN ausearch -k sigtrace_pcap -ts "$SCRIPT_START_TIME" 2>&1 | tee "$AUDIT_RAW_LOG" >/dev/null
    # 生成简要摘要：谁向谁发送了SIGTERM
    echo "==== SIGTERM发送摘要 ====" > "$AUDIT_SUMMARY_LOG"
    $SUDO_BIN ausearch -k sigtrace_pcap -ts "$SCRIPT_START_TIME" 2>/dev/null \
...
# 记录系统信息
echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 开始socat连接调试" | tee -a "$MAIN_LOG"
...
monitor_network() {
    while true; do
        echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): TCP连接状态" >> "$NETWORK_LOG"
        ss -tn | awk -v ts="$(date +'%Y-%m-%d %H:%M:%S.%3N')" '{print ts " ss: " $0}' | grep -E "8900|5555" >> "$NETWORK_LOG"
        echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 活跃连接详情" >> "$NETWORK_LOG"
        netstat -antp | awk -v ts="$(date +'%Y-%m-%d %H:%M:%S.%3N')" '{print ts " netstat: " $0}' | grep -E "8900|5555" >> "$NETWORK_LOG"
        echo "---" >> "$NETWORK_LOG"
        sleep 1
    done
}
...
start_python_reader() {
    ...
    echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 启动Python读取进程（$FIFO_PATH）..." | tee -a "$MAIN_LOG"
...
cleanup() {
    ...
    # 收集最终状态
    echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 最终网络状态:" | tee -a "$MAIN_LOG"
    ss -tn | grep -E "8900|5555" | tee -a "$MAIN_LOG"
    
    echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 最终进程状态:" | tee -a "$MAIN_LOG"
    ps aux | grep -E "socat|python3.*recover_audio_streaming" | grep -v grep | tee -a "$MAIN_LOG"
    ...
    echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 调试会话结束" | tee -a "$MAIN_LOG"
}
...
echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 进入socat循环（仅提供管道，自动重启）..." | tee -a "$MAIN_LOG"
while true; do
    echo "$(date +%Y-%m-%d\ %H:%M:%S.%3N): 启动socat监听8900 -> FIFO($FIFO_PATH)" | tee -a "$MAIN_LOG"
    if command -v ts >/dev/null 2>&1; then
        strace -ff -tt -T -e trace=network,read,write,close -o "$STRACE_LOG" \
        socat -d -d -d -d TCP-LISTEN:8900,reuseaddr,so-keepalive=1,keepidle=15,keepintvl=5,keepcnt=6 \
